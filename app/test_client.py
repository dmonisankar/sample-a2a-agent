...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test client for A2A Currency Agent')
    parser.add_argument(
        'base_url', 
        nargs='?',
        default='http://localhost:10000',
        help='Base URL of the agent (default: http://localhost:10000)'
    )
    parser.add_argument(
        '--deployed',
        action='store_true',
        help='Use the latest deployed Code Engine URL'
    )
    ...
    if args.deployed:
        base_url = 'https://currency-agent-new5-app.1yep95hyxos1.us-east.codeengine.appdomain.cloud'
        print(f"🚀 Using deployed agent URL: {base_url}")
    else:
        base_url = args.base_url
        if base_url == 'http://localhost:10000':
            print(f"🏠 Using local development URL: {base_url}")
        else:
